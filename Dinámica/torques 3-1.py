...
# =============================================================================
# 11. SINERGIA DE TORQUES CON SVD (UN SOLO ACTUADOR)
# =============================================================================

# Matriz de torques
Tau = tau_total.copy()   # tamaño (3 x N)

# Aplicar SVD
U, S, Vt = np.linalg.svd(Tau, full_matrices=False)

# Primera sinergia (más importante)
B = U[:, 0].reshape(3,1)


print("\n========== 1 actuador ==========")
print("\nMatriz B (sinergia dominante):")
print(B)

# Señal del actuador único
u = B.T @ Tau

# Reconstrucción de torques
Tau_aprox = B @ u

# =============================================================================
# 11.1 TORQUE DEL ACTUADOR ÚNICO (u)
# =============================================================================
...
plt.plot(t, u.flatten(), label='Torque actuador único')

# Puntos máximos y mínimos
plt.scatter(t[i_max_u], u_max, color='green', label='Máximo')
plt.scatter(t[i_min_u], u_min, color='red', label='Mínimo')

plt.xlabel('Tiempo (s)')
plt.ylabel('Torque (Nm)')
plt.title('Torque del actuador único (sinergia)')
plt.legend()
plt.grid()
plt.show()
...
print("\nTorque máximo actuador único:", u_max)
print("\nTorque con 30% :  ", u_max*1.3)

# =============================================================================
# 12. RMS DEL ACTUADOR ÚNICO
# =============================================================================
...
# RMS acumulado
u_rms_time = np.sqrt(np.cumsum(u**2) / np.arange(1, len(t)+1))

# Máximo del RMS acumulado
u_rms_max = np.max(u_rms_time)
i_max_rms = np.argmax(u_rms_time)

print("RMS acumulado máximo:", u_rms_max)

plt.figure()
plt.plot(t, u_rms_time, label='RMS acumulado')

# Punto máximo
plt.scatter(t[i_max_rms], u_rms_max, color='red', label='Máximo')

plt.xlabel('Tiempo (s)')
plt.ylabel('Torque RMS acumulado')
plt.title('Evolución del esfuerzo del actuador único')
plt.legend()
plt.grid()
plt.show()
# =============================================================================
# 13. ERROR DE APROXIMACIÓN
# =============================================================================

error = Tau - Tau_aprox

# RMS por articulación
RMS_error = np.sqrt(np.mean(error**2, axis=1))

print("\n===== ERRORES Y CALIDAD =====")
print("\nRMS de error por articulación:")
print("MCP:", RMS_error[0])
print("PIP:", RMS_error[1])
print("DIP:", RMS_error[2])

# RMS total
RMS_total = np.sqrt(np.mean(error**2))
print("\nRMS total del error:", RMS_total)


# =============================================================================
# 14. CALIDAD DE LA APROXIMACIÓN
# =============================================================================

energia_total = np.sum(Tau**2)
energia_error = np.sum(error**2)

calidad = 1 - (energia_error / energia_total)

print("\nCalidad de aproximación (0 a 1):", calidad)
...
# ---- Torques originales vs aproximados ----
plt.figure(figsize=(12,8))

plt.subplot(3,1,1)
plt.plot(t, Tau[0,:], label='Original MCP')
plt.plot(t, Tau_aprox[0,:], '--', label='Aprox MCP')
plt.legend(); plt.grid()

plt.subplot(3,1,2)
plt.plot(t, Tau[1,:], label='Original PIP')
plt.plot(t, Tau_aprox[1,:], '--', label='Aprox PIP')
plt.legend(); plt.grid()

plt.subplot(3,1,3)
plt.plot(t, Tau[2,:], label='Original DIP')
plt.plot(t, Tau_aprox[2,:], '--', label='Aprox DIP')
plt.legend(); plt.grid()

plt.suptitle('Comparación torques originales vs actuador único')
plt.xlabel('Tiempo (s)')
plt.show()
...
plt.plot(t, error[0,:], label='Error MCP')
plt.plot(t, error[1,:], label='Error PIP')
plt.plot(t, error[2,:], label='Error DIP')

plt.title('Error de aproximación con un solo actuador')
plt.xlabel('Tiempo (s)')
plt.ylabel('Error (Nm)')
plt.legend()
plt.grid()
...
# ---- RMS acumulado ----
RMS_acum = np.zeros((3,len(t)))

for k in range(len(t)):
    RMS_acum[:,k] = np.sqrt(np.mean(error[:,:k+1]**2, axis=1))

plt.figure(figsize=(10,6))
plt.plot(t, RMS_acum[0,:], label='MCP')
plt.plot(t, RMS_acum[1,:], label='PIP')
plt.plot(t, RMS_acum[2,:], label='DIP')

plt.title('RMS acumulado del error')
plt.xlabel('Tiempo (s)')
plt.ylabel('RMS (Nm)')
plt.legend()
plt.grid()
